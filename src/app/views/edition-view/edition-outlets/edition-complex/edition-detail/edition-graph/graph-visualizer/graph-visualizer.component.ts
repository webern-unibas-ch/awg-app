...
export class GraphVisualizerComponent implements OnInit {
     ...
     *
     * It keeps the result of the query as an observable of triples, QueryResult, or an info string.
     */
    queryResult$: Observable<Triple[] | QueryResult | string>;
     ...
     *
     * @returns {Promise<Triple[] | QueryResult | string>} The result of the query.
     */
    private async _queryLocalStore(
        queryType: string,
        queryString: string,
        triples: string
    ): Promise<Triple[] | QueryResult | string> {
        // Capture start time of query
        const t1 = Date.now();

        let result: Triple[] | QueryResult | string;
